...
'''
python GeneAnnotate.py Aligned.sortedByCoord.out.bam Aligned.sortedByCoord.out.bam.featureCounts ann.bam
'''
}
...
umi_tools count --umi-tag=Jr --cell-tag=Jp --per-gene --gene-tag=XT --skip-tags-regex=NA --per-cell -I ann.bam -S counts.txt --extract-umi-method=tag
'''
}
...
java -jar AlleleMASSeq.jar ann.bam new.vcf counts.txt
'''
